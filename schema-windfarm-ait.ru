...
delete {graph ?graph_schema_windfarm_ait_URL {?_s_ ?_p_ ?_o_}}
where {
  {select * { service <http://localhost:7333/repositories/ontorefine:data> {
    bind(iri(concat("graph/schema/windfarm-ait")) as ?graph_schema_windfarm_ait_URL)
  }}}
...
insert {graph ?graph_schema_windfarm_ait_URL {
  ...
  ?schema_windfarm_ait_n_URL a csvw:Column, sosa:ObservableProperty;
    csvw:title ?title;
    dct:identifier ?n_xsd_integer;
    csvw:name ?name;
    un:influxDatatype ?influxDatatype;
    csvw:suppressOutput ?suppressOutput_xsd_boolean;
    csvw:datatype ?schema_windfarm_ait_n_dtBase_URL, ?xsd_datatype_URL;
  ...
  ?schema_windfarm_ait_n_dtBase_URL a csvw:Datatype;
...
where {
  service <http://localhost:7333/repositories/ontorefine:data> {
    ...
    bind(iri(concat("schema/windfarm-ait/",?n)) as ?schema_windfarm_ait_n_URL)
    bind(strdt(?n,xsd:integer) as ?n_xsd_integer)
    bind(strdt(?suppressOutput,xsd:boolean) as ?suppressOutput_xsd_boolean)
    bind(iri(concat("schema/windfarm-ait/",?n,"/",?dtBase)) as ?schema_windfarm_ait_n_dtBase_URL)
