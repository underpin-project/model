...
delete {graph ?graph_schema_windfarm_generator2_URL {?_s_ ?_p_ ?_o_}}
where {
  service <http://localhost:7333/repositories/ontorefine:schema-windfarm-generator2> {
    bind(iri(concat("graph/schema/windfarm-generator2")) as ?graph_schema_windfarm_generator2_URL)
  }
...
insert {graph ?graph_schema_windfarm_generator2_URL {
  ...
  ?schema_windfarm_generator2_n_URL a csvw:Column, sosa:ObservableProperty;
    csvw:title ?title;
    dct:identifier ?n_xsd_integer;
    csvw:name ?name;
    un:influxDatatype ?influxDatatype;
    csvw:suppressOutput ?suppressOutput_xsd_boolean;
    csvw:datatype ?schema_windfarm_generator2_n_dtBase_URL, ?datatype;
  ...
  ?schema_windfarm_generator2_n_dtBase_URL a csvw:Datatype;
    csvw:base ?dtBase;
    csvw:format ?format.
}}
where {
  service <http://localhost:7333/repositories/ontorefine:schema-windfarm-generator2> {
    ...
    bind(iri(concat("schema/windfarm-generator2/",?n)) as ?schema_windfarm_generator2_n_URL)
    bind(strdt(?n,xsd:integer) as ?n_xsd_integer)
    bind(strdt(?suppressOutput,xsd:boolean) as ?suppressOutput_xsd_boolean)
    bind(iri(concat("schema/windfarm-generator2/",?n,"/",?dtBase)) as ?schema_windfarm_generator2_n_dtBase_URL)
